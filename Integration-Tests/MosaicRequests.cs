using io.nem2.sdk.Infrastructure.HttpRepositories;
using io.nem2.sdk.Model.Accounts;
using io.nem2.sdk.src.Infrastructure.HttpRepositories;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Reactive.Linq;
using System.Text;
using System.Threading.Tasks;

namespace Integration_Tests
{
    internal class MosaicRequests
    {

        [SetUp]
        public void Setup()
        {
        }


        [Test, Timeout(20000)]
        public async Task SearchMosaics()
        {
            var metadataHttp = new MosaicHttp("75.119.150.108", 3000);

            var queryModel = new QueryModel(QueryModel.DefineRequest.SearchMosaics);
            queryModel.SetParam(QueryModel.DefinedParams.ownerAddress, Address.CreateFromHex("68258605CB5ABC592FE691190202CDFD6DDEE659A6BB30B8").Plain);

            var response = await metadataHttp.SearchMosaics(queryModel);

            Assert.That(response.Data[0].Id, Is.EqualTo("6644D77F079630C9338F7048"));
            Assert.That(response.Data[0].MosaicInfo.Id, Is.EqualTo("6BED913FA20223F8"));
            Assert.That(response.Data[0].MosaicInfo.Version, Is.EqualTo(1));
            Assert.That(response.Data[0].MosaicInfo.Supply, Is.EqualTo(8391424283926782));
            Assert.That(response.Data[0].MosaicInfo.StartHeight, Is.EqualTo(1));
            Assert.That(response.Data[0].MosaicInfo.OwnerAddress, Is.EqualTo("68258605CB5ABC592FE691190202CDFD6DDEE659A6BB30B8"));
            Assert.That(response.Data[0].MosaicInfo.Revision, Is.EqualTo(1));
            Assert.That(response.Data[0].MosaicInfo.Flags, Is.EqualTo(2));
            Assert.That(response.Data[0].MosaicInfo.Divisibility, Is.EqualTo(6));
            Assert.That(response.Data[0].MosaicInfo.Duration, Is.EqualTo(0));
        }

        [Test, Timeout(20000)]
        public async Task SearchMosaic()
        {
            var metadataHttp = new MosaicHttp("75.119.150.108", 3000);

            var queryModel = new QueryModel(QueryModel.DefineRequest.SearchMosaics);
            queryModel.SetParam(QueryModel.DefinedParams.ownerAddress, Address.CreateFromHex("68258605CB5ABC592FE691190202CDFD6DDEE659A6BB30B8").Plain);

            var response = await metadataHttp.GetMosaic("6BED913FA20223F8");

            Assert.That(response.Id, Is.EqualTo("6644D77F079630C9338F7048"));
            Assert.That(response.MosaicInfo.Id, Is.EqualTo("6BED913FA20223F8"));
            Assert.That(response.MosaicInfo.Version, Is.EqualTo(1));
            Assert.That(response.MosaicInfo.Supply, Is.EqualTo(8391424283926782));
            Assert.That(response.MosaicInfo.StartHeight, Is.EqualTo(1));
            Assert.That(response.MosaicInfo.OwnerAddress, Is.EqualTo("68258605CB5ABC592FE691190202CDFD6DDEE659A6BB30B8"));
            Assert.That(response.MosaicInfo.Revision, Is.EqualTo(1));
            Assert.That(response.MosaicInfo.Flags, Is.EqualTo(2));
            Assert.That(response.MosaicInfo.Divisibility, Is.EqualTo(6));
            Assert.That(response.MosaicInfo.Duration, Is.EqualTo(0));
        }

        [Test, Timeout(20000)]
        public async Task GetMosaics()
        {
            var metadataHttp = new MosaicHttp("75.119.150.108", 3000);

            var response = await metadataHttp.GetMosaics(new List<string> { "63078E73FBCC2CAC", "6BED913FA20223F8" });

            Assert.That(response[0].Id, Is.EqualTo("6644D78C079630C9338F734F"));
            Assert.That(response[0].MosaicInfo.Id, Is.EqualTo("63078E73FBCC2CAC"));
            Assert.That(response[0].MosaicInfo.Version, Is.EqualTo(1));
            Assert.That(response[0].MosaicInfo.Supply, Is.EqualTo(3800000));
            Assert.That(response[0].MosaicInfo.StartHeight, Is.EqualTo(117));
            Assert.That(response[0].MosaicInfo.OwnerAddress, Is.EqualTo("6854F763D03307D0281EFC4BB3B4926316F9AEE74EB63EE8"));
            Assert.That(response[0].MosaicInfo.Revision, Is.EqualTo(1));
            Assert.That(response[0].MosaicInfo.Flags, Is.EqualTo(3));
            Assert.That(response[0].MosaicInfo.Divisibility, Is.EqualTo(0));
            Assert.That(response[0].MosaicInfo.Duration, Is.EqualTo(0));

            Assert.That(response[1].Id, Is.EqualTo("6644D77F079630C9338F7048"));
            Assert.That(response[1].MosaicInfo.Id, Is.EqualTo("6BED913FA20223F8"));
            Assert.That(response[1].MosaicInfo.Version, Is.EqualTo(1));
            Assert.That(response[1].MosaicInfo.Supply, Is.EqualTo(8391424283926782));
            Assert.That(response[1].MosaicInfo.StartHeight, Is.EqualTo(1));
            Assert.That(response[1].MosaicInfo.OwnerAddress, Is.EqualTo("68258605CB5ABC592FE691190202CDFD6DDEE659A6BB30B8"));
            Assert.That(response[1].MosaicInfo.Revision, Is.EqualTo(1));
            Assert.That(response[1].MosaicInfo.Flags, Is.EqualTo(2));
            Assert.That(response[1].MosaicInfo.Divisibility, Is.EqualTo(6));
            Assert.That(response[1].MosaicInfo.Duration, Is.EqualTo(0));
        }

        [Test, Timeout(20000)]
        public async Task GetMosaicMerkle()
        {
            var metadataHttp = new MosaicHttp("75.119.150.108", 3000);

            var response = await metadataHttp.GetMosaicMerkle( "6BED913FA20223F8" );

            Assert.That(response.Raw, Is.EqualTo(""));
        }
    }
}
